...
class CoinStoreController extends GetxController {
  ...
  Future<void> getDepositMethod(String payableAmount) async {
    amountController.text = payableAmount;
    currency = coinStoreRepo.apiClient.getCurrencyOrUsername();
    methodList.clear();
    methodList.add(paymentMethod!);

    ResponseModel responseModel = await coinStoreRepo.getDepositMethods();
  ...
  Future<void> submitDeposit(String coinId) async {
    ...
    ResponseModel responseModel = await coinStoreRepo.insertDeposit(amount: amount, methodCode: paymentMethod?.methodCode ?? "", currency: paymentMethod?.currency ?? "", coinID: coinId);
